...
**Detecting drift at scale.** Right now, drift shows up as a `hard_failure` with the exact step and locator chain that failed — the raw material for a reviewer to write an override, but not (in this repo) an automated suggestion loop. At real scale, the natural next step is exactly what stretch goal "confidence & approval" describes: track a per-capability, per-tenant replay success rate, and when a specific tenant's success rate drops, surface it as a drift alert with the failing step attached, rather than waiting for someone to notice.

## 5. Escalation & handoff

One function, `raiseIntervention()`, is the entire handoff mechanism, called identically whether discovery is stuck (`request_human`, 3 consecutive tool failures, or the step budget is exhausted) or replay hits a risky step or an unrecoverable condition. The browser runs **headful** specifically so this works: when automation cedes control, it simply stops issuing Playwright commands, writes an `InterventionRequest` (reason, goal, current step, screenshot, page URL) to `evidence/`, and blocks on a terminal prompt. The human takes the real, visible Chrome window directly — same OS process, same Playwright `page`, same cookies/session, not a fresh one. On resume, a second screenshot and the operator's notes are recorded, and automation continues with a fresh observation of whatever the human left behind. Control is tracked explicitly (`controlState: "agent" | "human"` in the record), so "who's driving" is never ambiguous.

**Scope note (as the brief explicitly allows):** this is single-machine, same-process handoff — sufficient to make the mechanism and the control-transfer model real, but not a remote multi-operator console. The documented extension is `chromium.launchServer()` in place of `chromium.launch()`: the main process keeps its `wsEndpoint`, writes it into the intervention record instead of just pausing, and a separate operator-facing process connects via `chromium.connect(wsEndpoint)` to the same live browser — the control-transfer *model* (ownership state, pause/resume, evidence capture) doesn't change, only how the human physically reaches the window.
...
- **Cross-tenant drift detection is a single hand-authored heuristic** (one known label difference), not a general UI-diffing engine. Real scale needs either a human-in-the-loop review queue or a success-rate/confidence signal (see §4) — deliberately not built, since the brief asks not to reward premature scaling infrastructure.
- **The operator "console" is a terminal prompt**, not a UI — intentional, since the brief explicitly allows mocking this as long as the handoff mechanism itself is real.
- **What I'd build next**: a confidence/approval gate on unattended replay (stretch goal, not attempted here — cut in favor of depth on the artifact schema and error taxonomy, which the brief weights higher), and the `chromium.launchServer()` remote-operator extension described in §5.
